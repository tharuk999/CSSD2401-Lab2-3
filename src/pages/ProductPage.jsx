...
const BOOK = {
  ...
  rating: 4.8,
  reviews: 2341,
  description: 'A powerful primer on how design serves as the communication between object and user -- and how to make that communication work. Revised and expanded edition.',
...
export default function ProductPage({ onAddToCart }) {
  ...
  function handleBuyNow() { onAddToCart(BOOK); navigate('/cart') }
  ...
  return (
    <main id="main-content" className="max-w-5xl mx-auto px-5 py-14" tabIndex={-1}>
      ...
      <div aria-hidden="true" className="pointer-events-none fixed inset-0 overflow-hidden">
        ...
        <div className="absolute bottom-1/3 right-1/4 w-80 h-80 rounded-full opacity-6"
      ...
      <div className="relative z-10 grid md:grid-cols-2 gap-14 items-center">
        ...
        <div className="page-enter stagger-1 flex justify-center">
          <div
            className="animate-float-book relative w-56 h-80 rounded-xl select-none"
            style={{
              background: 'linear-gradient(135deg, #1a1c34 0%, #0d0e1f 55%, #22253f 100%)',
              boxShadow: '0 30px 80px rgba(0,0,0,0.7), 4px 0 0 rgba(240,160,0,0.6), inset 0 1px 0 rgba(240,160,0,0.1)',
            }}
            aria-hidden="true"
          >
            {/* Spine accent */}
            <div className="absolute left-0 top-0 bottom-0 w-3 rounded-l-xl"
              style={{ background: 'linear-gradient(180deg, #f0a000 0%, #f5b835 40%, #f0a000 100%)' }} />
            {/* Top rule */}
            <div className="absolute top-6 left-5 right-5 h-px bg-amber-500/60" />
            {/* Content */}
            <div className="absolute inset-0 flex flex-col justify-between px-6 py-8">
              <div>
                <p className="font-display text-amber-400 text-xs tracking-widest uppercase mb-3 font-medium">
                  Don Norman
                </p>
                <h2 className="font-display text-white text-2xl leading-tight font-light italic">
                  The Design of<br />Everyday<br />
                  <span className="not-italic font-semibold">Things</span>
                </h2>
              </div>
              <div>
                <div className="w-10 h-px bg-amber-500/70 mb-3" />
                <p className="font-body text-night-200 text-xs tracking-widest uppercase">Basic Books</p>
              </div>
            </div>
            {/* Gloss */}
        ...
        <div className="space-y-6">
          ...
          {/* Rating */}
          <div className="page-enter stagger-2 flex items-center gap-3"
            aria-label={`Rated ${BOOK.rating} out of 5 from ${BOOK.reviews.toLocaleString()} reviews`}>
            <div className="flex gap-0.5" aria-hidden="true">
              {[1,2,3,4,5].map(s => (
                <svg key={s} className={`w-5 h-5 ${s <= Math.round(BOOK.rating) ? 'text-amber-400' : 'text-night-500'}`}
          ...
          <div className="page-enter stagger-3 flex items-baseline gap-4">
            <span className="font-display text-5xl text-amber-400 font-light">${BOOK.price.toFixed(2)}</span>
          ...
          <div className="page-enter stagger-4 flex flex-col sm:flex-row gap-3 pt-1">
            <button
              ...
              aria-label="Add The Design of Everyday Things to cart"
            >
              {added ? (
                <>
            ...
            <button
              ...
              aria-label="Buy The Design of Everyday Things now"
            >
              Buy Now
            </button>
          </div>
          ...
          <div role="status" aria-live="polite" aria-atomic="true" className="sr-only">
            {added ? 'The Design of Everyday Things has been added to your cart.' : ''}
          </div>
        </div>
      </div>
